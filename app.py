...
from extensions import CustomExceptions, CurrencyExchange
...
def conversion(message: telebot.types.Message):
    try:
        values_in_text = message.text.split()

        if len(values_in_text) != 3:
            raise CustomExceptions('Введи /команду или три параметра.')

        quote, base, amount = values_in_text
        course = CurrencyExchange.convert(quote, base, amount)
        ...
        text = f'Цена {amount} {quote}  - {round(course*int(amount), 4)} {base}'
